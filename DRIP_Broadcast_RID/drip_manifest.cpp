...
bool drip_manifest_build(DRIPManifestState *state,
    ...
    memcpy(&payload[41], det,      DET_BYTES);                 // [41..56] UA DET (16)
    ...
    memcpy(&signed_data[40], det,      DET_BYTES);

    bool sign_ok = det_sign(signed_data, sizeof(signed_data), &payload[57]); // [57..120] Sig (64)
    if (!sign_ok) {
