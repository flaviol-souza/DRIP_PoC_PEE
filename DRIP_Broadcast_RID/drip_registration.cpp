...
// DET derivation now uses the single shared det_compute() from det_generator
...
static BroadcastEndorsement g_be_hda_ua;     // child = UA,   parent = HDA
...
static void make_be(BroadcastEndorsement *be,
    ...
    Ed25519::sign(be->sig, parent_seed, parent_pub, region, sizeof(region));
}
...
void drip_reg_init(const uint8_t ua_det[DET_BYTES],
    ...
    // BE: HDA,UA — child = UA, signed by HDA
    make_be(&g_be_hda_ua,   vnb, vna, ua_det,  ua_hi,   det_hda,  HDA_SEED,  hda_pub);

    g_reg_ready = true;

    Serial.print(F("[REG] Apex DET: ")); for (int i=0;i<DET_BYTES;i++) Serial.printf("%02X", det_apex[i]); Serial.println();
    Serial.print(F("[REG] RAA  DET: ")); for (int i=0;i<DET_BYTES;i++) Serial.printf("%02X", det_raa[i]);  Serial.println();
    Serial.print(F("[REG] HDA  DET: ")); for (int i=0;i<DET_BYTES;i++) Serial.printf("%02X", det_hda[i]);  Serial.println();
    Serial.printf("[REG] 3-link chain signed. VNB=%u VNA=%u\n", vnb, vna);
}

const BroadcastEndorsement *drip_reg_next_link(void) {
    if (!g_reg_ready) return nullptr;
    ...
    uint8_t sel = pattern[idx];
    idx = (idx + 1) % 6;

    switch (sel) {
        case 1:  return &g_be_raa_hda;
        case 2:  return &g_be_apex_raa;
        default: return &g_be_hda_ua;
    }
}

uint8_t drip_reg_hda_ua_sam(uint8_t out[DRIP_LINK_SAM_BYTES]) {
    if (!g_reg_ready) { memset(out, 0, DRIP_LINK_SAM_BYTES); return 0; }
    drip_link_serialize_sam(&g_be_hda_ua, out);
